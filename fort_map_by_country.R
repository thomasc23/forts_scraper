# Animated Map of Forts Over Time - Colored by Country
# Creates an animated ggplot showing fort locations appearing by year, colored by nationality
...
# Extract primary nationality (first one if multiple)
# and create cleaner country categories
forts <- forts %>%
  mutate(
    # Get first nationality from pipe-separated list
    primary_nationality = sapply(strsplit(nationality, "\\|"), `[`, 1),
    # Create cleaner country categories
    country = case_when(
      grepl("United States", primary_nationality) ~ "United States",
      primary_nationality == "Great Britain" ~ "Great Britain",
      primary_nationality == "France" ~ "France",
      primary_nationality == "Spain" ~ "Spain",
      primary_nationality == "Mexico" ~ "Mexico",
      primary_nationality == "Russia" ~ "Russia",
      primary_nationality == "Netherlands" ~ "Netherlands",
      primary_nationality == "Sweden" ~ "Sweden",
      TRUE ~ "Other"
    )
  )
...
cat("\nFiltered to", nrow(forts_continental), "continental US forts\n")
...
# Create year bins - focus on 1600-1950 range
...
# Create decade bins for animation (25-year bins)
forts_continental <- forts_continental %>%
  mutate(decade = floor(earliest_year / 25) * 25)
...
# Define country colors (historically-inspired)
country_colors <- c(
  "United States" = "#3C3B6E",      # Navy blue (US flag)
  "Great Britain" = "#C8102E",      # British red
  "France" = "#0055A4",             # French blue
  "Spain" = "#F1BF00",              # Spanish gold
  "Mexico" = "#006847",             # Mexican green
  "Russia" = "#D52B1E",             # Russian red
  "Netherlands" = "#FF6600",        # Dutch orange
  "Sweden" = "#006AA7",             # Swedish blue
  "Other" = "#808080"               # Gray
)

# Set factor levels for consistent ordering in legend
forts_cumulative$country <- factor(
  forts_cumulative$country,
  levels = c("United States", "Great Britain", "France", "Spain",
             "Mexico", "Russia", "Netherlands", "Sweden", "Other")
)

forts_continental$country <- factor(
  forts_continental$country,
  levels = c("United States", "Great Britain", "France", "Spain",
             "Mexico", "Russia", "Netherlands", "Sweden", "Other")
)
...
p <- ggplot() +
  ...
  # Fort locations as points colored by country
  geom_point(
    data = forts_cumulative,
    aes(x = lon, y = lat, color = country),
    size = 1.5,
    alpha = 0.7
  ) +
  ...
  labs(
    ...
    subtitle = "Cumulative forts established, colored by founding nation",
...
# Save animation
anim_save("fort_animation_by_country.gif", animation = anim)
cat("\nAnimation saved to: fort_animation_by_country.gif\n")

# Also create a static map showing all forts by country
...
p_static <- ggplot() +
  ...
  labs(
    title = "North American Forts by Founding Nation (1600-1950)",
    subtitle = paste(nrow(forts_continental), "forts with geocoded locations"),
    caption = "Data: northamericanforts.com"
  )

ggsave("fort_map_by_country_static.png", p_static, width = 12, height = 8, dpi = 150)
cat("Static map saved to: fort_map_by_country_static.png\n")
